...
public class Simulation {
	...
	/**
	 * This is the lock that I will use to coordinate customer's entry to the
	 * coffee shop.
	 * 
	 */
	final static Object customerEntryToCoffeeShop = new Object();
	/**
	 * To keep track of number of tables
	 */
	static int numberOfTablesAcquired;
	/**
	 * Creating a machine for each foot type
	 */
	static Machine burgerMachine;
	static Machine coffeeMachine;
	static Machine friesMachine;
	
	static Thread[] cooks;
	
	/**
	 * Object to controll customer entry
	 */
	
	final static CustomerEntryControl CUSTOMER_ENTRY_CONTROL = new CustomerEntryControl();
	...
	public static List<SimulationEvent> runSimulation(int numCustomers, int numCooks, int numTables,
		...
		Simulation.CUSTOMER_ENTRY_CONTROL.setAvailable(true);
		Simulation.CUSTOMER_ENTRY_CONTROL.setFreeTables(numTables);
...
//		coffeeMachine = new Machine("CoffeeMaker2000", FoodType.coffee, machineCapacity);
	  ...
	  cooks = new Thread[numCooks];
		...
		{
			cooks[i] = new Thread(new Cook("Cook"+(i+1), true));
		}
